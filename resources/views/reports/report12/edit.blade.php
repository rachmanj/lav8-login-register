            ...
            <div class="card">
                ...
                <div class="card-body">
                    <dl class="row">
                        ...
                        <dd class="col-sm-8">:
                            {{ $invoice->inv_date ? date('d-M-Y', strtotime($invoice->inv_date)) : ' - ' }}</dd>
                        <dt class="col-sm-4">Vendor</dt>
                        <dd class="col-sm-8">: {{ $invoice->vendor->vendor_name }}</dd>
                        <dt class="col-sm-4">Amount</dt>
                        <dd class="col-sm-8">:
                            {{ $invoice->inv_nominal ? $invoice->inv_currency . '  ' . number_format($invoice->inv_nominal, 0) : '-' }}
                        </dd>
                ...
                <div class="card-footer">
                    <button class="btn btn-sm btn-primary mx-2" data-toggle="modal" data-target="#modal-upload"><i
                            class="fas fa-upload"></i> Attach File</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="modal-upload">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('reports.report12.update', $invoice->inv_id) }}" enctype="multipart/form-data"
                    method="POST">
                    @csrf @method('PUT')
